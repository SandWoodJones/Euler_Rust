...
enum Direction {
	...
	Left,
	Right,
	Dul, // diagonal upper left
	Dur, // diagonal upper right
	Dll, // diagonal lower left
	Dlr, // diagonal lower right
}
...
fn get_collection_from_direction(init_x: u32, init_y: u32, dir: &Direction, len: u32, width: u32, height: u32) -> Vec<(u32, u32)> {
	let mut r = vec![(init_x, init_y)];

	for i in 0 .. len - 1 {
		let (cur_x, cur_y) = r[i as usize];
		match get_index_from_dir(cur_x, cur_y, dir, width, height) {
			Some((new_x, new_y)) => r.push((new_x, new_y)),
			None => return r
		}
	}
